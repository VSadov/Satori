...
void SatoriRecycler::AddEphemeralRegion(SatoriRegion* region)
{
    ...
    {
        region->ClearMarks();
        // nothing marked -> nothing dead
        region->SetMayHaveDeadObjects(false);
    }
    ...
    // real occupancy will be known once we sweep.
    // for now assume the region is completely full
    region->ReportOccupancy(0);
...
void SatoriRecycler::HelpOnce()
{
    if (m_gcState != GC_STATE_NONE)
    {
        ...
        {
            bool revisit = HelpImpl();
            // if done helping, try initiating the blocking stage
            if (!revisit &&
...
{
    HelpOnce();
    if (m_regionsAddedSinceLastCollection > m_gen1Threshold)
    {
...
void SatoriRecycler::Collect(int generation, bool force, bool blocking)
{
    ...
    while (blocking && (collectionNum == collectionNumRef))
    {
        TryStartGC(generation);
        HelpOnce();
        //TODO: VS this could be a long pause for the caller if no helpers
        //      when GC is parallel, consider initiating blocking GC
        //      after a few helps and if the right generation
    }
}
...
void SatoriRecycler::BlockingCollect()
{
    ...
    {
        int gen1CountAtLastCollection = Gen1RegionCount() - m_regionsAddedSinceLastCollection;
        _ASSERTE(gen1CountAtLastCollection >= 0);
        m_gen1Budget -= gen1CountAtLastCollection;
    }

    m_gen1Threshold = max(GEN1_MIN_THRESHOLD, Gen2RegionCount() / 4);
...
void SatoriRecycler::MarkFn(PTR_PTR_Object ppObject, ScanContext* sc, uint32_t flags)
{
    ...
    _ASSERTE(location != (size_t)DEBUG_DestroyedHandleValue);
...
bool SatoriRecycler::DrainMarkQueuesConcurrent(SatoriMarkChunk* srcChunk, int64_t deadline)
{
    ...
    while (srcChunk)
    {
        ...
        {
            // if trying to block for GC, do not be a long pole
            if (m_gcState == GC_STATE_BLOCKING)
            {
        ...
        // - amortize cost of QueryPerformanceCounter()
        // - establish the minimum amount of work per help quantum
        //
        //TUNING: I picked this to be roughly just less than 0.1 msec on my machine
        if (count > Satori::REGION_SIZE_GRANULARITY / 512)
        {
            count = 0;
...
void SatoriRecycler::SweepNurseryRegions()
{
    ...
    while (curRegion = m_nurseryRegions->TryPop())
    {
        ...
        // now, after marking, we do not know this until we sweep
...
void SatoriRecycler::RelocateRegion(SatoriRegion* relocationSource)
{
    ...
    do
    {
        ...
        if (obj->IsMarked())
        {
            ...
            // MayHaveDeadObjects requires that all live objects are marked.
            if (relocationTarget->MayHaveDeadObjects())
            {
                ((SatoriObject*)dstPtr)->SetMarkedAtomic();
            }
    ...
    size_t freeSpace = copySize - (dstPtr - dstPtrOrig);
    if (freeSpace)
    {
        //TODO: VS consider owning, we would not need atomic mark as well
        //      but won't be able to share the dst region.
        // not returning to free list here since we do not own the region.
        SatoriObject::FormatAsFree(dstPtr, freeSpace);
    }
...
void SatoriRecycler::FinishRegions(SatoriRegionQueue* queue)
{
    ...
    while (curRegion = queue->TryPop())
    {
        if (m_isCompacting)
        {
            //TODO: VS perhaps can sweep on the way?
