...
namespace System.Net.Security
{
    internal static class SslStreamPal
    {
        ...
        private static SecurityStatusPal HandshakeInternal(SafeFreeCredentials credential, ref SafeDeleteSslContext context,
            ReadOnlySpan<byte> inputBuffer, ref byte[] outputBuffer, ref int outputSize2, SslAuthenticationOptions sslAuthenticationOptions)
        {
            ...
            catch (Exception exc)
            {
                ...
                if (size > 0)
                {
                    outputBuffer = size == output.Length ? output : new Span<byte>(output, 0, size).ToArray();
                }
                outputSize2 = size;
